...
import { daysOfTheWeek, getDaysForWeeksInMonth, segmentIntoWeeks } from '../../utils/util'
...
const CalendarCellWrap = styled.div`
    ...
    pointer-events: ${props => (props.isSunday || props.isSaturday || !props.isCurrentMonth) ? 'none;' : 'auto;'};
            ...
            return (
              <CalendarRow key={i}>
                {week.map((dayMoment, j) => {
                  ...
                  const isCurrentMonth = dayMoment.isSame(currentMonthMoment, 'month')
                  return (
                    <CalendarCellWrap
                      isSunday={isSunday}
                      ...
                      onClick={() => !isSunday && isCurrentMonth && onCellClicked(
                        dayMoment.format('DD'),
                        dayMoment.format('MM'),
